...
const MetricCard = ({ label, value, apoio, tone = 'from-sky-500/30 to-transparent', icone: Icone }) => (
    <article className="relative overflow-hidden border border-zen-border bg-zen-surface">
        ...
        <div className="flex items-start justify-between gap-3 p-4">
            ...
            {Icone && (
                <div className="shrink-0 bg-zen-bg/80 p-2 border border-zen-border">
...
const EmptyPanel = ({ title, description, actionLabel, onAction }) => (
    <div className="border border-dashed border-zen-border bg-zen-bg/40 px-6 py-12 text-center">
        <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center border border-zen-border bg-zen-surface">
        ...
        {actionLabel && onAction && (
            <button
                type="button"
                onClick={onAction}
                className="mt-5 inline-flex items-center gap-2 bg-zen-blue px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-blue-600"
            >
    ...
    return (
        <div className="grid gap-2">
            {epics.map((epic) => {
                ...
                return (
                    <article key={epic.id} className="overflow-hidden border border-zen-border bg-zen-surface">
                        {/* Epic header */}
                                        ...
                                        <div key={feature.id} className="px-4 py-3">
                                            ...
                                            {storyList.length === 0 ? (
                                                ...
                                                <div className="flex flex-wrap gap-1.5 ml-1">
                                                    {storyList.map((story) => (
                                                        <span key={story.id} className="inline-flex items-center gap-1.5 border border-amber-500/20 bg-amber-500/10 px-2.5 py-0.5 font-mono text-[10px] text-amber-100">
                        ...
                        upcomingItems.map((item) => (
                            <div key={item.id} className="border border-zen-border bg-zen-bg/50 p-3">
                                ...
                                <div className="mt-1.5 flex items-center gap-2">
                                    <span className="border border-zen-border bg-zen-surface px-2 py-0.5 font-mono text-[10px] text-zen-text-sec">{item.stageLabel}</span>
                                    {Number(item.percentual_progresso) > 0 && (
                                        <span className={`px-2 py-0.5 font-mono text-[10px] ${getProgressTone(Number(item.percentual_progresso))}`}>
...
const GanttPanel = ({ items, onOpenCard }) => {
    ...
    const getBarMetrics = (item) => {
        ...
        return { item, isBug: normalizeKey(item.alocado) === 'bugproj', progress, leftPct, widthPct }
    }
    ...
    return (
        <article className="overflow-hidden border border-zen-border bg-zen-surface">
            ...
            <div className="zen-scroll overflow-x-auto pb-2" style={{ scrollbarWidth: 'thin', scrollbarColor: '#334155 #0f172a' }}>
                        ...
                        <div>
                            <div className="grid border-b border-zen-border" style={{ gridTemplateColumns: `repeat(${totalDays}, minmax(0, 1fr))` }}>
                                {sprintHeaders.map((sprint) => (
                                    <div key={`${sprint.label}-${sprint.span}`} className="border-r border-zen-border/60 px-2 py-2 text-center font-mono text-[9px] font-semibold uppercase tracking-[0.22em] text-sky-300" style={{ gridColumn: `span ${sprint.span}` }}>
                                        {sprint.label}
                                    </div>
                                ))}
                            </div>
                            ...
                            <div key={item.id} className="grid h-[88px] border-b border-zen-border/60" style={{ gridTemplateColumns }}>
                                ...
                                >
                                    <div className="flex flex-wrap items-center gap-1.5">
                                        <TypePill type={item.alocado} />
                                        <span className={`inline-flex items-center border px-1.5 py-0.5 font-mono text-[10px] ${getStateBadgeClass(item.posicaoKanban)}`}>{item.stageLabel}</span>
                                    </div>
                                ...
                                <div className="relative overflow-hidden bg-[#0d131d]">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => onOpenCard(item)}
                                        className={`absolute top-1/2 z-30 flex h-10 -translate-y-1/2 items-center gap-2 overflow-hidden border px-2.5 text-left shadow-md transition-transform hover:scale-[1.01] ${
                                            isBug
                                    ...
                                    >
                                        <span
                                            className={`absolute inset-y-0 left-0 ${isBug ? 'bg-rose-300/20' : 'bg-emerald-300/18'}`}
                                        ...
                                        {widthPct >= 8 && (
                                            <span className="relative z-10 ml-auto inline-flex shrink-0 bg-black/25 px-1.5 py-0.5 font-mono text-[10px] font-semibold">{progress}%</span>
                                        )}
                                    </button>
                                </div>
                            </div>
...
const BacklogPanel = ({
    ...
    return (
        <div className="overflow-hidden border border-zen-border bg-zen-surface">
            {/* Toolbar */}
            <div className="flex flex-wrap items-center gap-2 border-b border-zen-border bg-zen-bg/40 px-4 py-3">
                ...
                <button
                    ...
                    className={`border px-3 py-2 font-mono text-[10px] font-medium transition-colors ${
                        openOnly
                                                                            ...
                                                                            <div key={atividade.id} className="grid grid-cols-[minmax(300px,1.8fr)_1fr_0.9fr_0.8fr_0.8fr] border-b border-zen-border/30 px-4 py-2.5 transition-colors hover:bg-white/[0.025]">
                                                                                ...
                                                                                <div className="flex items-center">
                                                                                    {Number(atividade.percentual_progresso) > 0 ? (
                                                                                        <span className={`px-2 py-0.5 font-mono text-[10px] ${getProgressTone(Number(atividade.percentual_progresso))}`}>{atividade.percentual_progresso}%</span>
                                                                                    ) : (
...
const CadastroControlModal = ({
    ...
    return (
        <div className="fixed inset-0 z-[88] flex items-center justify-center p-4 animate-in fade-in duration-200">
            ...
            <div className="relative max-h-[90vh] w-full max-w-6xl overflow-y-auto border border-zen-border bg-zen-surface shadow-2xl">
                {/* Modal header */}
                ...
                <div className="space-y-3 p-6">
                    {filteredEpics.length === 0 ? (
                        ...
                        filteredEpics.map((epic) => {
                            const featureList = featuresByEpic.get(epic.id) || []
                            return (
                                <article key={epic.id} className="overflow-hidden border border-zen-border bg-zen-bg/20">
                                    {/* Epic row */}
...
const Projetos = () => {
    ...
    return (
        <div className="flex w-full max-w-none flex-col gap-5 p-4 sm:p-6 animate-in fade-in duration-300">
            ...
            {/* Toast flutuante */}
            {feedback && (
                <div className={`fixed bottom-5 right-5 z-50 flex max-w-sm items-start gap-3 border bg-zen-surface px-4 py-3 shadow-2xl shadow-black/50 animate-in slide-in-from-bottom-3 duration-300 ${
            ...
            <header className="relative overflow-hidden border border-zen-border bg-zen-surface">
                {/* accent line + radial glow */}
                <div className="h-[2px] w-full bg-gradient-to-r from-sky-500/60 via-cyan-500/30 to-transparent" />
                <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,rgba(56,189,248,0.08),transparent_50%)]" />
                {/* corner decoration */}
                            ...
                            {acoesOrdenadas.map((acao) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`group inline-flex items-center gap-2 border ${acao.borda} ${acao.bg} px-3 py-2 font-mono text-[10px] font-semibold text-white transition-all hover:brightness-125 hover:shadow-lg`}
                                    >
            ...
            <div className="grid gap-5 xl:grid-cols-[1fr_280px]">
                        ...
                        {REPORT_TABS.map((tab) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`relative inline-flex items-center gap-2 px-4 py-3 font-mono text-[10px] font-semibold uppercase tracking-[0.18em] transition-colors ${
                                        active
                                            ? 'text-white after:absolute after:bottom-0 after:left-0 after:right-0 after:h-[2px] after:bg-zen-blue'
                                            : 'text-zen-text-sec hover:text-white'
                                    }`}
                                >
                                    <Icone className="h-3.5 w-3.5" />
                                    {tab.rotulo}
                                    <span className={`border px-1.5 py-0.5 font-mono text-[9px] font-bold leading-none ${
                                        active ? 'border-sky-500/30 bg-sky-500/20 text-sky-300' : 'border-zen-border/60 bg-transparent text-zen-text-tri'
                                    }`}>
                                        {tabCounts[tab.id]}
                                    </span>
                                </button>
                ...
                <aside className="xl:sticky xl:top-5 xl:self-start">
                    <OverviewRail upcomingItems={upcomingItems} metrics={metrics} />
                </aside>
            </div>

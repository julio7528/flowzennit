...
import { Image as ImageIcon, Loader2, Pencil, Plus, Trash2, Upload, Users, X } from 'lucide-react'
...
const CadParticipantes = () => {
  ...
  const loadParticipants = async (currentUserId) => {
    ...
    if (error) {
      setFeedback({ type: 'error', message: t('participants.feedback.loadError') })
      setParticipants([])
    } else {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      ...
      setFeedback({ type: 'error', message: t('participants.feedback.imageError') })
    } finally {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!nomeparticipante.trim()) {
      setFeedback({ type: 'error', message: t('participants.feedback.nameRequired') })
      return
    }
    ...
    if (editingId) {
      ...
      if (error) {
        setFeedback({ type: 'error', message: t('participants.feedback.updateError') })
        setSaving(false)
        return
      }

      setFeedback({ type: 'success', message: t('participants.feedback.updateSuccess') })
    } else {
      ...
      if (error) {
        setFeedback({ type: 'error', message: t('participants.feedback.createError') })
        setSaving(false)
        return
      }

      setFeedback({ type: 'success', message: t('participants.feedback.createSuccess') })
    }
  ...
  const handleDelete = async (participantId) => {
    ...
    const confirmed = window.confirm(t('participants.feedback.deleteConfirm'))
    if (!confirmed) return
    ...
    if (error) {
      setFeedback({ type: 'error', message: t('participants.feedback.deleteError') })
      return
    }

    setFeedback({ type: 'success', message: t('participants.feedback.deleteSuccess') })
    await loadParticipants(userId)
  }
  ...
  return (
    <div className="flex w-full max-w-none flex-col gap-6 p-6 animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-display font-semibold text-white tracking-tight">{t('participants.title')}</h1>
          <p className="text-sm text-zen-text-sec mt-0.5">{t('participants.description')}</p>
        </div>
        <button
          onClick={() => {
            if (formOpen) {
        ...
        >
          ...
          {formOpen ? t('common.close') : t('participants.actions.new')}
        </button>
      </div>
      ...
      {formOpen && (
        ...
        >
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-5">
              <div className="relative size-16 rounded-full border border-zen-border/80 overflow-hidden flex items-center justify-center bg-zen-bg shadow-inner">
                {fotobase64 ? (
                  <img src={fotobase64} alt={t('participants.form.previewAlt')} className="h-full w-full object-cover" />
                ) : (
              ...
              <label className="flex items-center gap-2 px-4 py-2 bg-zen-bg border border-zen-border hover:border-zen-blue/50 rounded-lg text-sm text-white font-medium cursor-pointer transition-colors hover:bg-white/[0.02]">
                ...
                {t('participants.actions.chooseImage')}
              ...
              {fotobase64 && (
                ...
                >
                  {t('participants.actions.removeImage')}
                </button>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 pt-4 border-t border-zen-border/50">
            ...
            >
              {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : editingId ? t('participants.actions.update') : t('participants.actions.save')}
            </button>
            <button
              type="button"
              onClick={() => {
      ...
      <div className="bg-zen-surface border border-zen-border rounded-xl overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-zen-border bg-zen-surface/50">
          <h2 className="font-display font-medium text-sm text-white tracking-wide">{t('participants.list.title')}</h2>
        </div>
        ...
        <div className="divide-y divide-zen-border/50">
          <div className="grid grid-cols-[80px_1fr_100px] gap-4 px-6 py-3 text-xs font-semibold text-zen-text-tri uppercase tracking-wider bg-zen-bg/30">
            <span>{t('participants.list.profile')}</span>
            <span>{t('participants.list.name')}</span>
            <span className="text-right">{t('participants.list.actions')}</span>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center px-6 py-12 text-center">
              ...
              <p className="text-sm text-white font-medium">{t('participants.list.emptyTitle')}</p>
              <p className="text-sm text-zen-text-sec mt-1">{t('participants.list.emptyDescription')}</p>
            </div>
          ) : (
              ...
              >
                ...
                <div className="flex items-center justify-end gap-1">
                  <button
                    ...
                    title={t('participants.actions.edit')}
                  >
